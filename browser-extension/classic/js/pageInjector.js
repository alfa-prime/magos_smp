...
export async function injectionTargetFunction(enrichedDataForForm) {
  ...
  function waitForElementEnabled(doc, selector, timeout = 15000) {
    return new Promise((resolve, reject) => {
      const start = Date.now();
      (function check() {
        const el = doc.querySelector(selector);
  ...
  function waitForGridRowsSettled(doc, timeout = 30000, stableDelay = 1500) {
    return new Promise((resolve, reject) => {
  ...
  async function selectFromReferenceField({ doc, iframeWindow, fieldSelector, column, value }) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      if (attempt < maxRetries) {
        console.warn(`[pageInjector] Запись не найдена, ждем 2 секунды и пробуем снова...`);
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        try {
            await waitForRowSelected(firstRow, 'x-grid-row-selected', 1000);
